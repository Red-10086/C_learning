...
编写inverse_n函数,返回将无符号整数x的第pos位开始的pos位起n位取反后的值.
    unsigned inverse_n(unsigned x, int pos, int a){codes}
*/
...
int main(void)
{
    ...
    unsigned count = 0;/* 起始位 */
    unsigned width = 0;/* 修改位宽 */
    
    /* 打印提示信息和读输入 */
    printf("请输入一个非负整数:\n");
    scanf("%u", &number);
    printf("例如:数字 count 的二进制是 101,最低位(第1位)是1,第2位是0,第3位是1.\n");
    printf("请输入您想修改的起始位数(1~32):\n");
    scanf("%u", &count);
    printf("请输入您想修改的位宽:\n");
    scanf("%u", &width);
    ...
    printf("无符号整数%u:\n",number);
    print_bits(number);
    printf("无符号整数%u的第%u位起共%u位设为1后的值:\n", number, count, width);
    print_bits(set_n(number, count, width));
    printf("无符号整数%u的第%u位起共%u位设为0后的值:\n", number, count, width);
    print_bits(reset_n(number, count, width));
    printf("无符号整数%u的第%u位起共%u位取反后的值:\n", number, count, width);
    print_bits(inverse_n(number, count, width));
    return 0;
}
